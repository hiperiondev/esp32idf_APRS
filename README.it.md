...
- **Soft-modem sul chip.** AFSK 1200 Bd Bell 202 (APRS standard) con fino a sei demodulatori in parallelo (un set multicomparatore che condivide prefiltri inclinati, regolabile dall'amministrazione web), più AFSK 1200 Bd V.23, AFSK 300 Bd e **G3RUH 9600 Bd FSK** — tutto in C puro sull'ADC/DAC dell'ESP32 stesso.
...
- **Ricevitore GNSS e tracciamento live.** Un modulo NMEA sulla propria UART (RMC/GGA/GSA/GSV/VTG, multi-costellazione) con interruttore principale, una pagina di vista live aggiornata una volta al secondo, e un controllo *Usa GPS* che riempie i campi di posizione di qualunque pagina con il fix corrente. Il beacon Tracker può trasmettere il fix live invece di una posizione fissa, con lo SmartBeaconing che rende l'intervallo adattivo alla velocità e lo anticipa in curva.
- **Risponditore di interrogazioni APRS.** Risponde alle interrogazioni generali `?APRS?`/`?WX?`/`?IGATE?`/`?QRU?` e all'insieme diretto (`?APRSD`/`?APRSH`/`?APRSM`/`?APRSO`/`?APRSP`/`?APRSS`/`?APRST`/`?PING?`), ciascuna con limiti di frequenza per tipo e per sorgente, più un beacon periodico opzionale delle capacità di stazione.
...
- **Bot Telegram.** Un bot opzionale accanto ai servizi APRS — long polling, autorizzazione per utente e per chat, inoltro a Telegram dei messaggi di stazione e dei bollettini, e un pulsante Mini App — per controllare leggermente la stazione dal telefono.
...
- **Pannello web, 22 pagine.** Autenticazione HTTP Basic con due account — uno di amministratore e uno facoltativo di sola lettura che vede ogni pagina, non cambia nulla e può solo avviare e fermare il visore del registro di console —, una dashboard in tempo reale con log del traffico in tempo reale (long-poll JSON) e un feed JSON degli ultimi ascoltati, gestione file LittleFS (upload/download/eliminazione/formattazione), Wi-Fi AP/STA/AP+STA con scansione e controllo della potenza di TX, controllo della frequenza della CPU (80/160/240 MHz), e un visore del registro di console su richiesta che copia l'uscita seriale nel browser. Tutta l'interfaccia è responsiva da un solo foglio di stile — su tablet o telefono la barra laterale diventa un cassetto scorrevole, le righe del modulo si riducono a una colonna, le tabelle larghe scorrono dentro la pagina e i controlli crescono a una dimensione tattile.
...
| Area | Note |
|---|---|
| AFSK 1200 Bd Bell 202 | Fino a sei demodulatori in parallelo, profilo predefinito |
...
| Interconnessione APRS BrandMeister | Riconoscimento, gating e instradamento dei messaggi sulla sessione APRS-IS esistente; nessun collegamento DMR |
...
| Bot Telegram | Long polling, autorizzazione per utente/chat, inoltro messaggi e bollettini, pulsante Mini App |
...
| Ricevitore GNSS (NMEA, UART propria) | Interruttore principale, pagina di vista live, riempimento posizione *Usa GPS* su ogni pagina |
| Tracciamento GPS live + SmartBeaconing | Solo beacon Tracker; intervallo adattivo alla velocità e anticipo in curva |
| Risponditore di interrogazioni APRS | Generali `?APRS?`/`?WX?`/`?IGATE?`/`?QRU?` + insieme diretto, con limiti di frequenza; beacon delle capacità |
...
| Pannello web | 22 pagine, dashboard live, log del traffico in tempo reale + feed degli ultimi ascoltati, visore del registro di console, aiuto contestuale per opzione |
...
| Localizzazione | EN / ES / IT, a compile-time, etichette e fumetti di aiuto allo stesso modo |
...
```bash
# Richiede ESP-IDF v6.1 (testato e fissato a 6.1)
