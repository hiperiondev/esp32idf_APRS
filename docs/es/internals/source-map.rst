...
aproximados. El C de primera parte suma ~69 k líneas entre ``main/`` +
``components/`` (excluyendo ``managed_components/``), de las cuales ~7,1 k son el
componente del módem y ~18,9 k la administración web.
...
.. code-block:: text
   ...
   │   ├── winlink/        ← correo por radio Winlink sobre APRSLink: máquina de estados de sesión, cola de órdenes, buzón (winlink.json propio)
   ...
   │   │       ├── bme280/bme280.c                ← controlador I2C real BME280/BMP280 (por defecto)
   │   │       └── bmp180/bmp180.c                ← ídem, BMP180 más viejo (desactivado por defecto)
   ...
   │       ├── web_help.c              ← tabla etiqueta de opción → texto de ayuda, tras el signo
   │       │                             de interrogación que cierra cada etiqueta
