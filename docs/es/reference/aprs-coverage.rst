...
.. list-table::
   ...
   * - Identificador de versión de software en el destino (TOCALL)
     - ⚠️
     - Todo paquete originado usa ``APE32I`` (``APRS_TOCALL``). Es un TOCALL experimental válido, pero no está registrado en la lista ``aprsorg/aprs-deviceid``, así que los clientes receptores muestran la estación como una aplicación experimental genérica en vez de nombrar el firmware.
...
.. list-table::
   ...
   * - Identificador ``!`` tardío (excepción X1J)
     - ✅
     - Una posición sin marca temporal introducida por un ``!`` en cualquier lugar hasta la posición de carácter 40, que es lo que el capítulo permite para las tramas a las que un digipetidor X1J antepuso texto fijo, se clasifica y se decodifica desde ese desplazamiento. El barrido exige un campo de posición completo y bien formado antes de aceptar uno, así que un ``!`` dentro del texto de un comentario no se confunde con un identificador; el tráfico de tercero y los datos de prueba quedan fuera del barrido, así que la posición interna de un paquete retransmitido no se puede volver a gatear.
   * - Campo de comentario
     - ✅
...
.. list-table::
   ...
   * - Objetos de área
     - ✅
     - Forma, color y tamaño en las mismas ranuras de objeto, con las formas de línea capaces de declarar un ancho de corredor en millas como el token entre llaves al frente del comentario.
...
.. list-table::
   ...
   * - Datos de tormenta
     - ❌
     - Nada codifica ni decodifica la forma al aire. Una estación de este tipo no tiene de dónde sacar esa información —viene de servicios meteorológicos—, así que retransmitir esos paquetes intactos, que es lo que ocurre hoy, es la conducta sensata.
...
.. list-table::
   ...
   * - Marcador de no archivar
     - ✅
     - La cadena ``!x!`` que pide a las bases de datos detrás de APRS-IS no almacenar un paquete puede anteponerse a cada campo de texto libre propio - comentarios de posición, textos de estado, el comentario meteorológico, los comentarios de objetos e ítems y el texto de boletines - con una única casilla a nivel de estación (página Station), desactivada por defecto. El texto de mensajes, los paquetes de definición de telemetría y las respuestas a consultas quedan fuera de su alcance por diseño. Un campo que el operador ya marcó no se marca dos veces. Se dirige a los archivos, no a las pasarelas, así que nunca decide por dónde puede viajar una trama ni si llega a RF/APRS-IS; los paquetes retransmitidos no se ven afectados por esta opción y conservan el marcador que ya traía la estación de origen, si lo traía, porque la carga se pasa byte a byte.
...
.. list-table::
   ...
   * - Precedencia de símbolos
     - ✅
     - Las tres fuentes se consultan en recepción en el orden que fija el capítulo. El campo de información gana siempre que entregue un símbolo: todos los formatos de posición, objeto e ítem, comprimidos o no, y Mic-E, cuyo par de símbolo viaja ahí aunque su posición no. Una carga útil que no trae símbolo propio, sobre todo una sentencia NMEA cruda, recae en la dirección de destino y después, solo para ese tipo de dato, en el SSID de origen. Mic-E queda excluido del paso de la dirección de destino, porque ahí lleva latitud, código de mensaje y ambigüedad, no un símbolo. Un único resolvedor compartido atiende a las dos entradas, así que una estación se lee igual se la haya oído por el aire o llegado desde APRS-IS. En transmisión la cuestión no se plantea: el tráfico propio lleva siempre su símbolo en el campo de información.
