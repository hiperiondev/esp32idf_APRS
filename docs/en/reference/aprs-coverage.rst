...
.. list-table::
   ...
   * - No-archive marker
     - ✅
     - The ``!x!`` string that asks the databases behind APRS-IS not to store a packet can be prepended to every own-station free-text field - position comments, status texts, the weather comment, object and item comments and bulletin text - with a single station-wide checkbox (Station page), off by default. Message text, telemetry definition packets and query responses are outside its reach by design. A field the operator has already marked is not marked twice. It addresses the archives rather than the gateways, so it never decides where a frame may travel or whether it reaches RF/APRS-IS at all; relayed packets are unaffected by this setting and keep whatever marker, if any, the originating station already put there, since the payload is passed through byte for byte.
...
.. list-table::
   ...
   * - Symbol precedence
     - ✅
     - The three sources are consulted on receive in the order the chapter lays down. The information field wins whenever it yields a symbol — every position, object and item layout, compressed or not, and Mic-E, whose symbol pair travels there even though its position does not. A payload that carries no symbol of its own, a raw NMEA sentence above all, falls back to the destination address and then, for that data type alone, to the source SSID. Mic-E is excluded from the destination-address step, since its destination address carries latitude, message code and ambiguity rather than a symbol. One shared resolver serves both feeds, so a station reads the same whether it was heard on the air or arrived from APRS-IS. On transmit the question does not arise: originated traffic always carries its symbol in the information field.
   * - SSID conventions
     - ✅
