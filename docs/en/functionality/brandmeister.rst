...
transmitter and not only BrandMeister traffic. A worldwide monitor
subscription carries no geographic filter term of its own, so a BrandMeister
line that carries no position either — a repeater status broadcast, for
example — is refused by the position requirement beside that filter, whatever
that requirement is set to for ordinary traffic. See :ref:`en-igate` for the
whole INET→RF sequence. The *BrandMeister* page's status table reports the
range filter's state.
