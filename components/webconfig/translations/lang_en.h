...
#define TR_NOTE_RANGE_PREFIX "Local gate applied only to RF -> Internet, independent of the payload-type filter above. Range is measured from My Station's position; packets whose position can't be decoded are not affected by the range filter."

#define TR_F_3RDPARTY_UNWRAP_EN "Relay whitelisted third-party (}) traffic"
#define TR_NOTE_3RDPARTY_UNWRAP "Off by default. Only takes effect when the Internet to RF Callsign Filter above is set to Whitelist: a third-party-wrapped packet is only ever unwrapped and relayed if its inner source callsign is itself on the whitelist. Only enable if you trust and have whitelisted the specific source - re-gating third-party traffic without this restriction is the most common cause of IGate loops."
