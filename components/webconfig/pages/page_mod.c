 ...
 *
 * @brief Web admin "MOD (GPIO)" page: renders and saves the GPIO assignments,
 * offering only pins the modem accepts as PTT (output-capable, not colliding with
 * the ADC/DAC audio path, the input-only pads or the flash/PSRAM pads).
 */
...
esp_err_t page_mod_post(httpd_req_t *req) {
    if (!web_check_auth(req))
        return ESP_OK;
    char *body = malloc(WEBCONFIG_POST_BUF_MOD);
    if (!body) {
        httpd_resp_send_500(req);
        return ESP_OK;
    }
    if (web_read_body(req, body, WEBCONFIG_POST_BUF_MOD) < 0) {
        free(body);
        httpd_resp_send_500(req);
        return ESP_OK;
    }
