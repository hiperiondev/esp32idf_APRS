...
// with a space, '&', '"', or similar would silently mangle the generated
...
static void append_file_row(httpd_req_t *req, const char *name, long size) {
    ...
    size_t need = strlen(esc) * 2 + strlen(enc) + 400;
...
#pragma GCC diagnostic ignored "-Wformat-truncation"
    snprintf(row, need,
             ...
             "<form method='POST' action='/delete' style='display:inline' data-fname=\"%s\" "
             "onsubmit=\"return confirm('" TR_STORAGE_CONFIRM_DELETE_PREFIX "'+this.dataset.fname+'?');\">"
             "<input type='hidden' name='file' value='%s'>"
             "<button type='submit' class='btn danger'>" TR_STORAGE_DELETE "</button>"
             "</form></td></tr>",
             esc, size, enc, esc, esc);
