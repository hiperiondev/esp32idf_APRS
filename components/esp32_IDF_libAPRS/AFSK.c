...
static const char *TAG_AFSK = "AFSK";
...
// const float fir1200[] = {
//     0.00316367f, 0.00398861f, 0.00437723f, 0.00413208f, 0.00298872f, 0.00074047f, -0.00260284f, -0.00668306f, -0.01075057f, -0.01375085f, -0.01454411f,
//     -0.01221729f, -0.00641016f, 0.00244106f, 0.01303073f, 0.02333366f, 0.03097455f, 0.03375193f, 0.03020101f, 0.02005505f, 0.00447537f, -0.01403413f,
//     -0.03204008f, -0.04585163f, -0.05231281f, -0.04953725f, -0.03740889f, -0.01771846f, 0.00611527f, 0.02969454f, 0.04852908f, 0.05896765f, 0.05896765f,
//     0.04852908f, 0.02969454f, 0.00611527f, -0.01771846f, -0.03740889f, -0.04953725f, -0.05231281f, -0.04585163f, -0.03204008f, -0.01403413f, 0.00447537f,
//     0.02005505f, 0.03020101f, 0.03375193f, 0.03097455f, 0.02333366f, 0.01303073f, 0.00244106f, -0.00641016f, -0.01221729f, -0.01454411f, -0.01375085f,
//     -0.01075057f, -0.00668306f, -0.00260284f, 0.00074047f, 0.00298872f, 0.00413208f, 0.00437723f, 0.00398861f, 0.00316367f};
// const float fir2200[] = {
//     0.00231442f, -0.00057143f, -0.00355072f, -0.00511464f, -0.00386876f, 0.00049756f, 0.00630273f, 0.01005055f, 0.00823119f, -0.00000000f, -0.01113572f,
//     -0.01837338f, -0.01552207f, -0.00164028f, 0.01686771f, 0.02888214f, 0.02512596f, 0.00483548f, -0.02209390f, -0.03977362f, -0.03574035f, -0.00950762f,
//     0.02554950f, 0.04903640f, 0.04561719f, 0.01504976f, -0.02642793f, -0.05490854f, -0.05294572f, -0.02045258f, 0.02456674f, 0.05628144f, 0.05628144f,
//     0.02456674f, -0.02045258f, -0.05294572f, -0.05490854f, -0.02642793f, 0.01504976f, 0.04561719f, 0.04903640f, 0.02554950f, -0.00950762f, -0.03574035f,
//     -0.03977362f, -0.02209390f, 0.00483548f, 0.02512596f, 0.02888214f, 0.01686771f, -0.00164028f, -0.01552207f, -0.01837338f, -0.01113572f, -0.00000000f,
//     0.00823119f, 0.01005055f, 0.00630273f, 0.00049756f, -0.00386876f, -0.00511464f, -0.00355072f, -0.00057143f, 0.00231442f};
// // Sample 9600Hz
// //  const float fir1200[] = {
// //
...
void LED_Status2(uint8_t r, uint8_t g, uint8_t b) {
    ...
    if (r == r_old && g == g_old && b == b_old) {
        ...
        if (millis() > rgbTimeout) {
            ...
            b_old = b;
            if (_led_strip_pin > -1) {
                //ws2812_set_pixel(_led_strip_pin, r, g, b);
            } else {
                if (_led_tx_pin > -1) {
                    if (r > 0)
...
//   -0.001296,  -0.005406,  -0.012387,  -0.010745,  0.020421,  0.090370,  0.178421,  0.240624,  0.240624,  0.178421,  0.090370,  0.020421,  -0.010745,
//   -0.012387,  -0.005406,  -0.001296
// };
...
uint16_t CountOnesFromInteger16(uint16_t value) {
    uint16_t count = 0;
    for (int i = 0; i < 16; i++) {
        if (value & 0x0001)
            count++;
...
#define RTC_MODULE_CHECK(a, str, ret_val)                                                                                                                      \
    if (!(a)) {                                                                                                                                                \
        ESP_LOGD(TAG_AFSK, "%s:%d (%s):%s", __FILE__, __LINE__, __FUNCTION__, str);                                                                            \
        return (ret_val);                                                                                                                                      \
    }
...
static esp_err_t adc_set_i2s_data_pattern(adc_unit_t adc_unit, int seq_num, adc_channel_t channel, adc_bits_width_t bits, adc_atten_t atten) {
    ...
    RTC_MODULE_CHECK(atten <= ADC_ATTEN_DB_12, "ADC Atten Err", ESP_ERR_INVALID_ARG);
...
void I2S_Init(i2s_mode_t MODE, i2s_bits_per_sample_t BPS) {
    ...
    // {
    //   ESP_LOGD(TAG_AFSK, "using I2S_MODE");
    ...
    // {
    ESP_LOGD(TAG_AFSK, "Using I2S DAC/ADC_builtin");
    // install and start i2s driver
    if (i2s_driver_install(I2S_NUM_0, &i2s_config, 5, &i2s_event_queue) != ESP_OK) {
        ESP_LOGD(TAG_AFSK, "ERROR: Unable to install I2S drives");
    }
    // GPIO36, VP
    // init ADC pad
...
void AFSK_TimerEnable(bool sts) {
    ...
    if (sts == true) {
        ...
        if (AdcHandle != NULL) {
            ...
            //  ESP_LOGD(TAG_AFSK, "SAR DIV=%d LEN=%d", SYSCON.saradc_ctrl.sar_clk_div, SYSCON.saradc_ctrl.sar1_patt_len);
...
void setPtt(bool state) {
    ESP_LOGD(TAG_AFSK, "PTT Pin: %d, Active: %d, State: %d", _ptt_pin, _ptt_active, state);
    if (state) {
        setTransmit(true);
#if defined(CONFIG_IDF_TARGET_ESP32S3)
...
void afskSetModem(uint8_t val, bool bpf, uint16_t timeSlot, uint16_t preamble, uint8_t fx25Mode) {
    ...
    if (audio_buffer == NULL) {
        ESP_LOGD(TAG_AFSK, "Error allocating memory for audio buffer");
        return;
    }
    ESP_LOGD(TAG_AFSK, "Modem: %d, SampleRate: %d, BlockSize: %d", ModemConfig.modem, SAMPLERATE, BLOCK_SIZE);
...
void adc_continue_init(void) {
    ...
    if (Err != ESP_OK) {
        ESP_LOGE(TAG_AFSK, "Pin %u is not ADC pin!", adc_pins[0]);
    }
    if (adc_unit != 0) {
    ...
    if (Err != ESP_OK)
        ...
        ESP_LOGD(TAG_AFSK, "ADC Continuous Configuration Done. SAMPLERATE: %d Hz", AdcConfig.sample_freq_hz);
        ...
        if (Err != ESP_OK)
            ...
            if (Err != ESP_OK) {
                ESP_LOGE(TAG_AFSK, "adc_continuous_register_event_callbacks failed!");
            }
...
//     .bitwidth = ADC_BITWIDTH_12,
// };
// ESP_LOGD(TAG_AFSK, "Creating ADC_UNIT_%d line cali handle", 1);
...
//
            ...
            ESP_LOGD(TAG_AFSK, "Creating ADC_UNIT_%d cerve cali handle", 1);
//
#endif
...
static void sigmadelta_init(void) {
    sdm_config_t sdm_cfg = {
        ...
        .sample_rate_hz = 96 * 8 * 1000, // equivalent to old prescale=96 setting
#ifdef CONFIG_IDF_TARGET_ESP32C3
        .gpio_num = GPIO_NUM_1, // GPIO1 is used for ESP32C3
#elif defined(CONFIG_IDF_TARGET_ESP32S3)
        .gpio_num = GPIO_NUM_2, // GPIO2 is used for ESP32S3
#else
...
// adc1_config_channel_atten(SPK_PIN, ADC_ATTEN_DB_12); //Input 3.3Vp-p,Use R 10K divider input power 3.3V
// esp_adc_cal_get_characteristics(V_REF, ADC_ATTEN_DB_12, ADC_WIDTH_BIT_12, &characteristics);
// ESP_LOGD(TAG_AFSK, "v_ref routed to 3.3V\n");
    ...
    setTransmit(false);
    ESP_LOGD(TAG_AFSK, "AFSK Hardware Initialized");
}
...
void AFSK_init(int8_t adc_pin, int8_t dac_pin, int8_t ptt_pin, int8_t sql_pin, int8_t pwr_pin, int8_t led_tx_pin, int8_t led_rx_pin, int8_t led_strip_pin,
    ...
    tp->cdt_led_pin = 2;
    tp->cdt_led_on = 2;
    ESP_LOGD(TAG_AFSK, "cdt_led_pin = %d, cdt_led_on = %d, port = %d", tp->cdt_led_pin, tp->cdt_led_on, tp->port);
// tp->ptt_pin = 12;
#ifdef FX25TNCR2
...
void AFSK_deinit() {
    ...
    if (AdcHandle != NULL) {
        adc_continuous_stop(AdcHandle);
        ESP_LOGD(TAG_AFSK, "ADC continuous stopped");
    }
    ...
    if (audio_buffer != NULL) {
        free(audio_buffer);
        audio_buffer = NULL;
        ESP_LOGD(TAG_AFSK, "Audio buffer freed");
    }
    ...
    if (AdcCaliHandle != NULL) {
        ...
        ESP_LOGD(TAG_AFSK, "ADC calibration handle (line fitting) deleted");
#else
        adc_cali_delete_scheme_curve_fitting(AdcCaliHandle);
        ESP_LOGD(TAG_AFSK, "ADC calibration handle (curve fitting) deleted");
#endif
        AdcCaliHandle = NULL;
    }
    ...
    if (AdcHandle != NULL) {
        ...
        ESP_LOGD(TAG_AFSK, "ADC continuous handle deleted");
        AdcHandle = NULL;
    }

    ESP_LOGD(TAG_AFSK, "AFSK deinitialization completed - ADC memory freed");
}
...
void AFSK_Poll(bool SA818, bool RFPower) {
    ...
    if (_sql_pin > -1) {                                  // Set SQL pin active
    ...
    if (hw_afsk_dac_isr) {
        ...
        //       {
        //         ESP_LOGD(TAG_AFSK, "I2S Write Error");
        //       }
        //     }
        //     // size_t writeByte;
        ...
        if (audio_buffer != NULL) {
            ...
            if (i2s_read(I2S_NUM_0, (char *)&pcm_in, (BLOCK_SIZE * sizeof(uint16_t)), &bytesRead, portMAX_DELAY) == ESP_OK) {
                ...
                mVsum = 0;
                mVsumCount = 0;
                // ESP_LOGD(TAG_AFSK, "%i,%i,%i,%i", pcm_in[0], pcm_in[1], pcm_in[2], pcm_in[3]);
            ...
            while (RingBuffer_Size(&fifo) >= BLOCK_SIZE) {
                ...
                if (mVsumCount > 0) {
                    ...
                    // ESP_LOGD(TAG_AFSK, "Audio dc_offset=%d mVrms=%d", offset, mVrms);
                }
