...
static const char *TAG_AX25 = "AX25";
...
void ax25_decode_default(uint8_t *buf, size_t len, uint16_t mVrms) {
    static AX25Msg msg;
...
void Ax25BitParse(uint8_t bit, uint8_t modem, uint16_t mV) {
        ...
        if (rx->rawData == 0x7E) // HDLC flag received
        {
            if (rx->rx == RX_STAGE_FRAME) // if we are in frame, this is the end of the frame
            {
                if (rx->frameIdx >= 17) // correct frame must be at least 17 bytes long (source+destination+control+CRC)
                {
                    ...
                    {
                        uint16_t i = 13;
                        // ESP_LOGD(TAG_AX25, "[%i]%s",rx->frameIdx,rx->frame);
                        ...
                        if (Ax25Config.allowNonAprs || (((rx->frame[i + 1] == 0x03) && (rx->frame[i + 2] == 0xF0)))) {
                            ...
                            if (rx->crc != lastCrc) // the other decoder has not received this frame yet, so store it in main frame buffer
                            {
                                ...
                                // ESP_LOGD(TAG_AX25, "Pkt=%d SND: peak=%d valley=%d level=%d",rx->frameIdx,peak,valley,level);
                                ...
                                if (!rxFrameBufferFull) // if enough space, store the frame
                                {
                                    ...
                                    ESP_LOGD(TAG_AX25, "Pkt=%d SND: peak=%d valley=%d level=%d", rx->frameIdx, rxFrame[rxFrameHead].peak,
                                             rxFrame[rxFrameHead].valley, rxFrame[rxFrameHead].level);
#ifdef ENABLE_FX25
                                    ...
                                    ESP_LOGW(TAG_AX25, "RX frame buffer full");
                                }
                            }
                        }
                    }
    ...
    static void transmitStart(void) {
        ...
        txDelayElapsed = 0;
        ModemTransmitStart();
        ESP_LOGD(TAG_AX25, "transmitStart");
    }
    ...
    void Ax25TransmitCheck(void) {
        ...
        if (txQuiet < millis()) // quit time has elapsed
        {
            ...
            } else // channel is busy
            {
                ...
                {
                    txQuiet = millis() + board_random(100, 1000); // try again after some random time
                    txRetries++;
                }
            }
        }
    ...
    void Ax25Init(uint8_t fx25Mode) {
        ...
        txInitStage = TX_INIT_OFF;
        txQuiet = (millis() + (Ax25Config.quietTime) + board_random(10, 200)); // calculate required delay
    }
    ...
    void Ax25TimeSlot(uint16_t ts) {
        if (ts > 0) {
            ...
            txQuiet = (millis() + (Ax25Config.quietTime) + board_random(100, 1000)); // calculate required delay
        } else {
            txQuiet = 0; // no delay
        }
    }
    ...
    void convPath(ax25header * hdr, char *txt, unsigned int size) {
        ...
        p = strpos(txt, '-');
        if (p > 0 && p < size) {
            for (i = 0; i < p; i++) { // Get CallSign/Path
                hdr->addr[i] = txt[i];
            }
            j = 0;
            for (i = p + 1; i < size; i++) { // get SSID
                //            if(txt[i]=='*') break;
                //            if(txt[i]==',') break;
                //            if(txt[i]==':') break;
                if (txt[i] < 0x30)
            ...
            for (i = 0; i < size; i++) { // Get CallSign/Path
                if (txt[i] == '*')
                    break;
                if (txt[i] == ',')
                    break;
