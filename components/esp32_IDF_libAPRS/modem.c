 ...
 * 6. TUNE is the PLL counter tuning coefficient. It is applied when there is a symbol change (as the symbol change should occur when the PLL counter crosses
 * zero)
 *
 * [       DCD OFF    *      |    DCD ON   ]
...
// PLL tick increment value.
// Computed as 2^32 / N so that the 32-bit PLL accumulator wraps once per N samples.
// When N == 1, 2^32 / N is exactly 2^32, which does not fit in the 32-bit accumulator
// and would truncate to 0 (no PLL advancement at all). In that case the correct
// "wrap on (almost) every sample" step is the largest representable 32-bit value instead.
#define PLL_STEP(n) ((uint32_t)((n) == 1 ? 0xFFFFFFFFu : (((uint64_t)1 << 32) / (n))))

#define PLL1200_STEP PLL_STEP(N1200)
#define PLL9600_STEP PLL_STEP(N9600)
#define PLL300_STEP  PLL_STEP(N300)
...
static int32_t modemFirFilter(struct Filter *filter, int32_t input) {
    int32_t out = 0;
...
uint8_t MODEM_BAUDRATE_TIMER_HANDLER(void) {
    ...
    if (ModemConfig.modem == MODEM_9600) {
        ...
        // sinwave = modemFirFilter(&demodState[0].lpf, sinwave);
    } else {
        if (currentSymbol) {
            phaseAcc += spaceStep;
        } else {
...
static int32_t demodulate(int16_t sample, struct DemodState *dem) {
    ...
    if (ModemConfig.modem != MODEM_9600) {
        ...
        {
            dem->correlatorSamples[dem->correlatorSamplesIdx++] = modemFirFilter(&dem->bpf, sample);
    ...
    return modemFirFilter(&dem->lpf, sample) > 0;
}
...
void ModemTransmitStart(void) {
    ...
    // dacEn=1;
    // adcEn=-1;
    ESP_LOGD(TAG_MODEM, "ModemTransmitStart");
}
...
void ModemTransmitStop(void) {
    ...
    ESP_LOGD(TAG_MODEM, "ModemTransmitStop");
}
...
void ModemInit(void) {
    ...
    ESP_LOGD(TAG_MODEM, "markStep %d spaceStep %d baudRateStep %d", markStep, spaceStep, baudRateStep);
