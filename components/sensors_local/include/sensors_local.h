 ...
 *
 * @note To add a new sensor family in the future (e.g. GPS, power/battery, air
 *       quality) append a new bit (e.g. @c SENSOR_LOCAL_DATA_GPS @c = @c 1u @c <<
 *       @c 2) and OR it into ::SENSOR_LOCAL_DATA_ALL. Nothing else in the
 *       registry needs to change - sensors_local_register() only requires a
 *       driver's capabilities to be non-zero, and any UI page filters the
...
struct sensor_local_driver {
     ...
     *        May be NULL for a driver that does not publish a properties
     *        descriptor; NULL is treated as
    ...
    bool initialized; /**< Registry-managed (do not set from a driver): set once init() has succeeded, or when there is no init(). */
    bool failed;      /**< Registry-managed (do not set from a driver): set if init() returned an error; such drivers are skipped. */
};
