...
typedef enum {
    ...
    SENSOR_LOCAL_TLM_DIGITAL_B2 = 1u << 6,  /**< Digital (binary) channel B2 (bit 6). */
    SENSOR_LOCAL_TLM_DIGITAL_B3 = 1u << 7,  /**< Digital (binary) channel B3 (bit 7). */
    SENSOR_LOCAL_TLM_DIGITAL_B4 = 1u << 8,  /**< Digital (binary) channel B4 (bit 8). */
    SENSOR_LOCAL_TLM_DIGITAL_B5 = 1u << 9,  /**< Digital (binary) channel B5 (bit 9). */
...
typedef struct {
    ...
    /**
     * @brief Optional suggested engineering unit label for each Telemetry
     *        ANALOG channel (A1-A5 only, indices 0..4 - digital B1-B8 slots
     *        [5..12] are unused/ignored), indexed the same way as
     *        ::tlm_channel_name. Used by the web admin Telemetry page to
     *        auto-fill the channel's "Unit" field when this driver is picked
     *        as the channel's Source. NULL/empty means "no suggestion" -
     *        the page leaves the Unit field as-is.
     */
    const char *tlm_ana_unit[SENSOR_LOCAL_TLM_CHANNEL_COUNT];
    /**
     * @brief Optional suggested raw-reading range for each Telemetry ANALOG
     *        channel (A1-A5 only, indices 0..4), indexed the same way as
     *        ::tlm_channel_name: @c tlm_ana_eng_min[i] / @c tlm_ana_eng_max[i]
     *        are the driver's typical minimum/maximum *engineering* (real
     *        world) value for that channel - e.g. -20.0 / 60.0 for a
     *        DegC temperature channel. Used by the web admin Telemetry page
     *        to auto-fill Raw Min=0 / Raw Max=255 and derive the standard
     *        APRS101 8-bit linear calibration a=0, b=(eng_max-eng_min)/255,
     *        c=eng_min when this driver is picked as the channel's Source.
     *        Leave both at 0 and the matching ::tlm_ana_eng_valid bit clear
     *        for "no suggestion".
     */
     ...
     *        analog bits, i.e. bit i set = index i valid) marking which
     *        ::tlm_ana_eng_min / ::tlm_ana_eng_max entries actually hold a
     *        driver-supplied suggestion, since 0.0/0.0 is itself a valid
     *        (if degenerate) engineering range and can't be used as the
     *        "unset" sentinel. Only analog bits (0..4) are meaningful.
     */
    uint16_t tlm_ana_eng_valid;
...
/**
 * @brief Fetch a driver's suggested Unit / Raw-Min-Eng / Raw-Max-Eng for one
 *        ANALOG telemetry channel (A1-A5, @p ana_idx 0..4), for auto-filling
 *        the web admin Telemetry page's per-channel Unit/calibration fields
 *        when that driver is picked as the channel's Source.
 *
 * @param props     Pointer to the driver's ::sensor_local_properties_t (may be NULL).
 * @param ana_idx   Analog channel index, 0..4 (A1..A5).
 * @param out_unit  Set to the suggested unit string, or NULL if none/invalid index.
 * @param out_min   Set to the suggested engineering minimum (only meaningful if the return value is true).
 * @param out_max   Set to the suggested engineering maximum (only meaningful if the return value is true).
...
static inline bool sensor_local_properties_tlm_ana_hint(const sensor_local_properties_t *props, unsigned ana_idx, const char **out_unit, float *out_min,
    ...
    if (out_max != NULL)
        *out_max = 0.0f;
    if (props == NULL || ana_idx >= 5u)
        return false;
    if (out_unit != NULL)
        *out_unit = props->tlm_ana_unit[ana_idx];
    bool valid = (props->tlm_ana_eng_valid & (1u << ana_idx)) != 0;
    if (valid) {
        if (out_min != NULL)
            *out_min = props->tlm_ana_eng_min[ana_idx];
        if (out_max != NULL)
            *out_max = props->tlm_ana_eng_max[ana_idx];
    }
    return valid;
}
