 ...
 *
 * @brief Compile-time board configuration for the modem: the audio DAC/ADC
 * pins, the optional PTT/LED control pins, the sample-rate/DSP parameters, and
 * the derived values and consistency checks computed from them.
 *
 * Every value that must be adapted to a particular board lives here. All values
 * may be overridden from the build system (for example
 * @c idf.py @c build @c -DMODEM_ADC_GPIO=32, or @c target_compile_definitions
 * in CMakeLists.txt).
 *
 * @note soc/soc_caps.h is included for @c SOC_ADC_DIGI_RESULT_BYTES /
 * @c SOC_ADC_DIGI_DATA_BYTES_PER_CONV, used by the derived-value checks at the
 * bottom of this file.
 */
 ...
 *
 * This is an internal radiomodem feature and, like ::MODEM_ADC_GPIO /
 * ::MODEM_DAC_GPIO, a fixed, compile-time-only board wiring choice - it is
 * NOT exposed as a runtime/web-admin setting. Any output-capable GPIO may be
 * used (0-19, 21-23, 25-27, 32, 33). GPIO34-39 are input-only and cannot be
 * used. GPIO6-11 (internal SPI flash/PSRAM) are never usable either - see
 * afsk_gpio_is_output_capable(). Not needed for the loopback test.
 *
 * Any other value produces a compile error further down in this file.
 */
 ...
 *
 * The 9600 Bd profile needs more than four ADC samples per symbol. At four
 * (38400 Hz) the demodulator's timing recovery cannot work: its sample instant
 * is quantised to one ADC period (25 % of a symbol) and decode()'s three-sample
 * majority vote spans 75 % of a symbol, so the vote window always reaches into
 * a transition. Host simulation of the real modem.c, with both real clocks and
 * the measured analogue path and NO noise, produces hard bit errors at every
 * sampling phase where the ADC instants line up with the DAC's update instants
 * - and since the two clocks differ by ~0.05 %, the alignment walks through
 * those phases every ~55 ms, several times per transmission. At 76800 the same
 * simulation gives zero bit errors at every phase and with up to 30 us of TX
 * edge jitter.
 *
 * The AFSK profiles are unaffected: they are demodulated at 9600 Hz through a
 ...
 *
 * Bounding the frame size and separating the cores are independent measures
 * and both are applied here - the frame size bounds the damage if anything
 * else ever takes a long critical section, the core split removes this
 * particular one entirely.
 *
...
/**
 * @note MODEM_PTT_GPIO is a fixed board-wiring choice, exactly like
 * ::MODEM_ADC_GPIO / ::MODEM_DAC_GPIO: -1 (disabled) is always allowed,
 * GPIO34-39 are input-only and cannot drive PTT, GPIO6-11 are the internal
 * SPI flash/PSRAM pads and are never brought out, and it must not collide with
 * the audio front-end's own pins. The checks below enforce this.
 */
#if MODEM_PTT_GPIO != -1
#if (MODEM_PTT_GPIO < 0) || (MODEM_PTT_GPIO > 39)
#error "MODEM_PTT_GPIO must be -1 (disabled) or a GPIO number in 0..39."
#endif
#if (MODEM_PTT_GPIO >= 34) && (MODEM_PTT_GPIO <= 39)
#error "MODEM_PTT_GPIO cannot be GPIO34-39: those pins are input-only and cannot drive PTT."
#endif
#if (MODEM_PTT_GPIO >= 6) && (MODEM_PTT_GPIO <= 11)
#error "MODEM_PTT_GPIO cannot be GPIO6-11: those pins are wired to the internal SPI flash/PSRAM."
#endif
#if MODEM_PTT_GPIO == MODEM_ADC_GPIO
#error "MODEM_PTT_GPIO collides with MODEM_ADC_GPIO: pick a different PTT pin."
#endif
#if MODEM_PTT_GPIO == MODEM_DAC_GPIO
#error "MODEM_PTT_GPIO collides with MODEM_DAC_GPIO: pick a different PTT pin."
#endif
#endif /* MODEM_PTT_GPIO != -1 */
...
/**
 * @note The two interrupts must not share a core (see ::MODEM_DAC_TIMER_CORE):
 * this is not a performance preference, it is the difference between the DAC
 * ISR being masked for the length of an ADC frame memcpy and not being masked
 * at all. The check below enforces this.
 */
