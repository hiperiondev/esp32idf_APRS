...
- **Soft-módem en el chip.** AFSK 1200 Bd Bell 202 (APRS estándar) con hasta seis demoduladores en paralelo (un juego multicomparador que comparte prefiltros inclinados, ajustable desde la administración web), más AFSK 1200 Bd V.23, AFSK 300 Bd y **G3RUH 9600 Bd FSK** — todo en C puro sobre el propio ADC/DAC del ESP32.
...
- **Respondedor de consultas APRS.** Contesta las consultas generales `?APRS?`/`?WX?`/`?IGATE?`/`?QRU?` y el conjunto dirigido (`?APRSD`/`?APRSH`/`?APRSM`/`?APRSO`/`?APRSP`/`?APRSS`/`?APRST`/`?PING?`), cada una con límites de tasa por tipo y por origen, más una baliza periódica opcional de capacidades de estación.
...
- **Panel web, 22 páginas.** Autenticación HTTP Basic con dos cuentas — una de administrador y otra opcional de solo lectura que ve todas las páginas, no cambia nada y solo puede iniciar y detener el visor de registro de consola —, un dashboard en vivo con registro de tráfico en vivo (long-poll JSON) y un feed JSON de últimos escuchados, gestión de archivos LittleFS (subir/descargar/borrar/formatear), Wi-Fi AP/STA/AP+STA con escaneo y control de potencia de TX, control de frecuencia de CPU (80/160/240 MHz), y un visor de registro de consola bajo demanda que copia la salida serie al navegador. Toda la interfaz es adaptable desde una sola hoja de estilos — en tableta o teléfono la barra lateral pasa a ser un cajón deslizante, las filas de formulario se reducen a una columna, las tablas anchas se desplazan dentro de la página y los controles crecen a un tamaño táctil.
...
| Área | Notas |
|---|---|
| AFSK 1200 Bd Bell 202 | Hasta seis demoduladores en paralelo, perfil por defecto |
...
| Respondedor de consultas APRS | Generales `?APRS?`/`?WX?`/`?IGATE?`/`?QRU?` + conjunto dirigido, con límite de tasa; baliza de capacidades |
...
| Panel web | 22 páginas, dashboard en vivo, registro de tráfico en vivo + feed de últimos escuchados, visor de registro de consola, ayuda contextual por opción |
